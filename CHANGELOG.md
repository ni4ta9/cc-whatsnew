...
- Configurable output directory via `whatsnew.outputDir` in `~/.claude/settings.json`
  - Supports `~` expansion for home directory paths
  - Falls back to current directory when not set (backward compatible)

### Fixed

- Fix infinite loop in Step 6 completion (duplicate AskUserQuestion removed)
